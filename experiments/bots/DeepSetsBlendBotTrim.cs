...
public class DeepSetsBlendBotTrim : AI
{
    ...
    double Evaluate(SeededGameState gameState, PlayerEnum playerID)
    {
        ...
        if (_perspectiveCheckLogsRemaining > 0)
        {
            ...
            BotLog.Write($"DeepSetsBlendBotTrim.PerspectiveCheck: playerID={playerID}, " +
    ...
    void ProceedTree(Move move)
    {
        ...
        catch (Exception ex)
        {
            if (!_firstProceedTreeExceptionLogged)
            {
                ...
                BotLog.Write($"DeepSetsBlendBotTrim.ProceedTree: caught exception, falling back to no tree reuse: {ex}");
            }
        }
        rootNode = null;
    }
    ...
    public override void PregamePrepare()
    {
        ...
        BotLog.Write($"DeepSetsBlendBotTrim.PregamePrepare: resolved ONNX model path='{resolvedPath}', exists={fileExists}");
        ...
        try
        {
            ...
            BotLog.Write($"DeepSetsBlendBotTrim.PregamePrepare: ONNX model loaded OK from '{resolvedPath}', " +
                         $"size={fileInfo.Length} bytes, sha256={sha256}");
        }
        catch (Exception e)
        {
            BotLog.Write($"DeepSetsBlendBotTrim.PregamePrepare: FAILED to load ONNX model from '{resolvedPath}' " +
    ...
    public override Move Play(GameState gameState, List<Move> possibleMoves_, TimeSpan remainingTime)
    {
        ...
        if (move is null)
        {
            ...
            if (!_firstNoIsomorphicMatchLogged)
            {
                ...
                BotLog.Write($"DeepSetsBlendBotTrim.Play: bestMove had no isomorphic match in possibleMoves -- " +
    ...
    public override void GameEnd(EndGameState state, FullGameState? finalBoardState)
    {
        ...
        BotLog.Write($"DeepSetsBlendBotTrim.GameEnd: evalFlipped={_evalFlippedCount}, evalNotFlipped={_evalNotFlippedCount}, " +
        ...
        BotLog.Write($"DeepSetsBlendBotTrim.Throughput: totalEvalCalls={_totalEvalCalls}, " +
        ...
        BotLog.Write($"DeepSetsBlendBotTrim.NoIsomorphicMatchFallback: count={_noIsomorphicMatchCount}");
