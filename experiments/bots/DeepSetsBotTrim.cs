...
// DeepSetsBotTrim: a Monte Carlo tree search agent whose leaf evaluation comes
...
// to load.
public class DeepSetsBotTrim : AI
{
    ...
    static double DeckValue(List<UniqueCard> allCards, double progress)
    {
        double value = 0;
    ...
    static double EvaluateHeuristic(SeededGameState gameState, PlayerEnum playerID)
    {
        ...
        value += DeckValue(allCards, progress);
        value -= DeckValue(allCardsEnemy, progress);
    ...
    void ProceedTree(Move move)
    {
        ...
        catch (Exception ex)
        {
            if (!_firstProceedTreeExceptionLogged)
            {
                ...
                BotLog.Write($"DeepSetsBotTrim.ProceedTree: caught exception, falling back to no tree reuse: {ex}");
            }
        }
        rootNode = null;
    }
    ...
    public override void PregamePrepare()
    {
        ...
        BotLog.Write($"DeepSetsBotTrim.PregamePrepare: resolved ONNX model path='{resolvedPath}', exists={fileExists}");
        ...
        try
        {
            ...
            BotLog.Write($"DeepSetsBotTrim.PregamePrepare: ONNX model loaded OK from '{resolvedPath}', " +
                         $"size={fileInfo.Length} bytes, sha256={sha256}");
        }
        catch (Exception e)
        {
            BotLog.Write($"DeepSetsBotTrim.PregamePrepare: FAILED to load ONNX model from '{resolvedPath}' " +
    ...
    public override Move Play(GameState gameState, List<Move> possibleMoves_, TimeSpan remainingTime)
    {
        ...
        if (move is null)
        {
            ...
            if (!_firstNoIsomorphicMatchLogged)
            {
                ...
                BotLog.Write($"DeepSetsBotTrim.Play: bestMove had no isomorphic match in possibleMoves -- " +
    ...
    public override void GameEnd(EndGameState state, FullGameState? finalBoardState)
    {
        ...
        BotLog.Write($"DeepSetsBotTrim.GameEnd: evalFlipped={_evalFlippedCount}, evalNotFlipped={_evalNotFlippedCount}, " +
        ...
        BotLog.Write($"DeepSetsBotTrim.Throughput: totalEvalCalls={_totalEvalCalls}, " +
        ...
        BotLog.Write($"DeepSetsBotTrim.NoIsomorphicMatchFallback: count={_noIsomorphicMatchCount}");
